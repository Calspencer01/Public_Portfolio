...
# Load in data
df_wages_men <- read.csv("wage_data/weekly_wage_men.csv") %>%
  dplyr::rename(date = DATE, CPI_wage = LEU0254631400Q)

# 84 instances
df_wages_men.len <- nrow(df_wages_men)

# Positions of sample data within dataset
train_indeces <- 1:(ceiling(df_wages_men.len*0.8))

# Positions of in sample forecasts within dataset
in_forecast <- (ceiling(df_wages_men.len*0.8)+1):df_wages_men.len

# Time series object for in sample forecasts
df_wages_men.ts <- ts(df_wages_men$CPI_wage[train_indeces]
                      , start = c(2000, 1)
                      , frequency = 4)

# Time series object for out of sample forecasts
df_wages_men.ts_full <- ts(df_wages_men$CPI_wage
                      , start = c(2000, 1)
                      , frequency = 4)

# Graphing time series ----
par(cex = 0.5)
plot(df_wages_men.ts, ylim = c(800, 1700), type = "p")
title("Quarterly median usual weekly nominal earnings (men)")

# Analyze ACF & PACF ----
acf_plot(df_wages_men.ts, max_lag = 40,  title = "quarterly median usual weekly nominal earnings (men)")

# Making Data Stationary ----
print("First Difference")
diff1 <- diff(df_wages_men.ts, lag = 1)
plot(diff1)
acf_plot(diff1, max_lag = 40, title = "first difference") 
# Possible first order AR / first order MA

print("Second Difference")
diff2 <- diff(diff1, lag = 4)
plot(diff2)
acf_plot(diff2, max_lag = 40, title = "second difference") 

print("Seasonal Difference")
diff3 <- diff(df_wages_men.ts, lag = 4)
plot(diff3)
acf_plot(diff3, max_lag = 40, title = "seasonal difference")
# First order AR & first order MA

# Test initial possibilities
scores <- get_AICs(df_wages_men.ts
                   , list(c(0,1,1,0,1,0) # Parsimonious
                          , c(1,0,0,1,1,1)
                          , c(0,1,0,1,1,1)
                          , c(0,1,1,1,1,1) # Best
                          , c(1,1,1,1,1,1)
...
model <- Arima(df_wages_men.ts, order=c(0,1,1), seasonal = list(order = c(1,1,1), period = 4))
model.fit <- as.vector(fitted(model))
model.res <-  as.vector(residuals(model))
model.aic <- aic_Jha(model, df_wages_men.ts) 
model.aic

model2 <- Arima(df_wages_men.ts, order=c(1,0,0), seasonal = list(order = c(1,1,1), period = 4))
model2.fit <- as.vector(fitted(model2))
model2.res <-  as.vector(residuals(model2))
model2.aic <- aic_Jha(model2, df_wages_men.ts) 
model2.aic
...
par(cex = 0.5)
plot(df_wages_men$CPI_wage, ylim = c(800, 1700), type = "p")
title("Modeling quarterly median usual weekly nominal earnings (men)")
lines(model.fit, col = "purple")

get.best.sarima(df_wages_men.ts, maxord = rep(1,6))
get.best.sarima(df_wages_men.ts, maxord = rep(2,6))
...
forecast_model_in.data <- df_wages_men$CPI_wage[train_indeces]
...
for (h1 in 1:length(in_forecast)){
  ...
  forecast_model_in.data <- forecast_model_in.data %>%
    c(df_wages_men$CPI_wage[in_forecast[h1]]) # Append next observation to model's training data
...
plot(ts(df_wages_men$CPI_wage, start = 2000, frequency = 4), type = "p", xlim = c(2000, 2023), ylim = c(800, 1700))
title("One-year ahead in-sample forecasts of median usual weekly nominal earnings (men)")
...
# Calculate residuals
res <- df_wages_men$CPI_wage[in_forecast] - forecast_model_in.mean
...
forecast_model_out <- Arima(df_wages_men.ts_full, order=c(0,1,1), seasonal = list(order = c(1,1,1), period = 4))
...
par(cex = 0.5)
plot(ts(df_wages_men$CPI_wage, start = 2000, frequency = 4), type = "p", xlim = c(2000, 2023),ylim = c(800, 1700))
title("Out-of-sample forecasts of median usual weekly nominal earnings (men)")
lines((fitted(forecast_model_out)), col = "purple")
lines(ts(c(rep(NA, df_wages_men.len),  forecast_model_out.mean),  start = 2000, frequency = 4), col = "red")
lines(ts(c(rep(NA, df_wages_men.len),  forecast_model_out.lower), start = 2000, frequency = 4), col = "blue")
lines(ts(c(rep(NA, df_wages_men.len),  forecast_model_out.upper), start = 2000, frequency = 4), col = "blue")
