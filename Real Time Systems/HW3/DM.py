...
# DM.py
# Deadline monotonic execution generator
...
def main():
    ...
    for t in range(max_t + 1):
        ...
        for i in range(min_n, max_n+1):
            ...
            for job in jobs[i]:
                ...
                if (job["release"] <= t and job["remaining_c"] > 0):
                    ...
                    if (min_task != -1):
                        # Get tasks of the current job and min job
                        job_task = tasks[job["task"]]
                        prev_min_job_task = tasks[jobs[min_task][min_job]["task"]]
                        # Select this job as min if it has a shorter deadline
                        if (job_task["deadline"] < prev_min_job_task["deadline"]):
                            min_task = i
                            min_job = job["num"]
        ...
        if min_task == -1:
            ...
            jobs[next_job["task"]][next_job["num"]]["remaining_c"] -= 1

            # Add execution to out data
            out_data = addExecution(out_data, next_job["task"], next_job["num"], t, t+1)
